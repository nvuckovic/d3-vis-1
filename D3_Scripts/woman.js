...
const generateChart = data => {
    ...
    const bubble = data => d3.pack()
        .size([width, height])
        .padding(2)(d3.hierarchy({ children: data }).sum(d => Math.pow(d.scoreWoman, 1.5))); // change score to show different charts
    ...
    const circle = node.append('circle')
        ...
        tooltip.select('img').attr('src', "../flag.jpg");
            ...
            rand = Math.floor(Math.random() * d.data.scoreWoman);
            ...
            let title = d.data.verb + ": " + d.data.articlesWoman[rand].title;
        ...
        .on('click', (e, d) => window.open(d.data.articlesWoman[rand].url));
