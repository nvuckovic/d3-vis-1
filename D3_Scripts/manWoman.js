...
const generateChart = data => {
    ...
    const bubble = data => d3.pack()
        .size([width, height])
        .padding(2)(d3.hierarchy({ children: data }).sum(d => Math.pow(d.scoreManWoman, 1.5))); // change score to show different charts
    ...
    const circle = node.append('circle')
        ...
        tooltip.select('img').attr('src', "../flag.jpg");
            ...
            rand = Math.floor(Math.random() * d.data.scoreManWoman);
            ...
            let title = d.data.verb + ": " + d.data.articlesManWoman[rand].title;
        ...
        .on('click', (e, d) => window.open(d.data.articlesManWoman[rand].url));
